...
DO $$ BEGIN
    IF NOT EXISTS (
        ...
        WHERE tablename = 'user_bookmarks' AND policyname = 'Users can manage their own bookmarks'
    ) THEN
...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'user_activity' AND policyname = 'Users can view their own activity'
    ) THEN
...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'user_activity' AND policyname = 'System can insert user activity'
    ) THEN
...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'user_saved_searches' AND policyname = 'Users can manage their own saved searches'
    ) THEN
...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'user_notifications' AND policyname = 'Users can manage their own notifications'
    ) THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_user_bookmarks_updated_at') THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_user_saved_searches_updated_at') THEN
...
DECLARE
    ...
    total_judge_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO null_slug_count FROM judges WHERE slug IS NULL;
    SELECT COUNT(*) INTO total_judge_count FROM judges;
    
    IF null_slug_count > 0 THEN
        RAISE WARNING 'Found % out of % judges without slugs - manual intervention may be required', null_slug_count, total_judge_count;
    ELSE
        RAISE NOTICE 'Successfully generated slugs for all % judges', total_judge_count;
    END IF;
END;
$$;
...
SELECT 
    ...
    COUNT(*) FILTER (WHERE slug IS NOT NULL) as judges_with_slugs,
    COUNT(*) FILTER (WHERE slug IS NULL) as judges_without_slugs
FROM judges;
