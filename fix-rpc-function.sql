-- Fix RPC function data type mismatch
-- The actual courts table uses VARCHAR(255) for name, not TEXT
...
RETURNS TABLE(
    court_id UUID,
    court_name VARCHAR(255),  -- Changed from TEXT to match actual schema
    court_type VARCHAR(50),   -- Likely varchar as well
    jurisdiction VARCHAR(2),  -- Likely varchar for state codes
    judge_count INTEGER,
    total_cases INTEGER,
