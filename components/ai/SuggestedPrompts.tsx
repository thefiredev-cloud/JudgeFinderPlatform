...
export default function SuggestedPrompts({ onSelectPrompt }: SuggestedPromptsProps) {
  ...
  return (
    <div className="px-3 sm:px-4 py-3 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
      ...
      <div className="flex flex-col sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
        {prompts.map((prompt, index) => (
          <button
            ...
            className="flex items-center gap-3 px-3 py-3 sm:py-2.5 text-xs bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md text-left touch-manipulation"
          >
            ...
            <span className="text-sm sm:text-xs text-slate-700 dark:text-slate-300 font-medium">
              {prompt.shortText}
            </span>
          </button>
        ))}
