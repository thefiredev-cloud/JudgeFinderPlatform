...
import { getCourtAdSpots } from '@/lib/ads/service'
...
export async function GET(
  ...
  if (!validation.success) {
    ...
    logger.apiResponse('GET', `/api/courts/${resolvedParams?.id ?? 'unknown'}/advertising-slots`, 400, Date.now() - start, {
      error: message
    })
  ...
  const courtId = validation.data.id
  logger.apiRequest('GET', `/api/courts/${courtId}/advertising-slots`)

  try {
    const spots = await getCourtAdSpots(courtId)
    ...
    logger.apiResponse('GET', `/api/courts/${courtId}/advertising-slots`, 200, Date.now() - start, {
      count: serialized.length
    })

    return response
  } catch (error) {
    logger.apiResponse('GET', `/api/courts/${courtId}/advertising-slots`, 500, Date.now() - start, {
